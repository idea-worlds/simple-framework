...
class Jackson2JsonsDelegate implements Jsons.JsonsDelegate {

    private final JsonMapper.Builder BUILDER;
    private final JsonMapper OBJECT_MAPPER;

    Jackson2JsonsDelegate() {
    ...
    @Override
    public Object objectMapper(boolean newInstance) {
        return newInstance ? BUILDER.build() : OBJECT_MAPPER;
    }

    @Override
    public void config(Consumer<Object> action) {
        action.accept(OBJECT_MAPPER);
    }

    @Override
    @SneakyThrows
    public <T> T read(Object obj, Class<T> clazz) {
        if (obj == null) {
            return null;
        }
    ...
    @Override
    @SneakyThrows
    public <T> T readWithGeneric(String json, Class<T> clazz, Class<?>... components) {
    ...
    @Override
    @SneakyThrows
    public void write(Object json, File out) {
        OBJECT_MAPPER.writeValue(out, json);
    }

    @Override
    @SneakyThrows
    public void write(Object json, OutputStream out) {
        OBJECT_MAPPER.writeValue(out, json);
    }

    @Override
    @SneakyThrows
    public void write(Object json, Writer out) {
        OBJECT_MAPPER.writeValue(out, json);
    }
